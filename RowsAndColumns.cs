...
{
    public partial class RowsAndColumns : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (pic == null)
            {
                ...
                firstSize = new PhotoSizeModel
                    (
                        width: measurementHelpers.ConvertToPixel(double.Parse(previous_SizeOneWidth)),
                        height: measurementHelpers.ConvertToPixel(double.Parse(previous_SizeOneHeight)),
                        row: double.Parse(previous_SizeOneRow),
                        column: double.Parse(previous_SizeOneColumn)
                    );
                secondSize = new PhotoSizeModel
                    (
                        width: measurementHelpers.ConvertToPixel(double.Parse(previous_SizeTwoWidth)),
                        height: measurementHelpers.ConvertToPixel(double.Parse(previous_SizeTwoHeight)),
                        row: double.Parse(previous_SizeTwoRow),
                        column: double.Parse(previous_SizeTwoColumn)
                    );
                pic = conversion.CreateImageByColumnsAndRows(resourceImg, pictureBox1, firstSize, secondSize, double.Parse(txtBorder.Text));
        ...
        private void RowsAndColumns_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!saveClicked)
            {
                var message = MessageBox.Show("Are you sure you want to close without saving changes?", "Confirmation", MessageBoxButtons.YesNo);
